...
class ComparisonVisualizer:
    ...
    def plot_metric_comparison_bar(
        ...
        for i, method in enumerate(methods):
            ...
            bars = ax.bar(
                x + offset, values, width, label=method, color=color, alpha=0.8
            )
    ...
    def plot_text_attribution_comparison(
        ...
        for ax, (method_name, attrs) in zip(axes, attributions.items()):
            ...
            _ = np.arange(len(filtered_tokens))
        ...
        sm.set_array([])
        cbar = plt.colorbar(
            sm, ax=axes, orientation="horizontal", fraction=0.05, pad=0.1, aspect=50
        )
    ...
    def plot_insertion_deletion_curves(
        ...
        axes[0].set_title(
            "Insertion Curve\n(Higher is Better)", fontsize=self.config.title_size
        )
        ...
        axes[1].set_title(
            "Deletion Curve\n(Lower is Better)", fontsize=self.config.title_size
        )
    ...
    def create_summary_dashboard(
        ...
        if text_results:
            ...
            ax3.set_title(
                "Text Attribution: IG vs DiET Token Overlap (SST-2)", fontweight="bold"
            )
            ax3.set_ylim(0, 1)
            ...
            ax4.bar(
                ["BERT Baseline"],
                [text_acc],
                color=self.method_colors["baseline"],
                alpha=0.8,
            )
        ...
        if image_results:
            ...
            if improvement > 0:
                summary_text += (
                    f"• Image: DiET improves attribution quality by {improvement:.4f}\n"
                )
            else:
                summary_text += (
                    f"• Image: GradCAM performs better by {-improvement:.4f}\n"
                )
...
"""
            ...
            baseline_acc_str = (
                f"{baseline_acc:.2f}%" if baseline_acc is not None else "N/A"
            )
...
"""
        ...
        if "text_experiments" in results:
            ...
            txt_baseline_acc_str = (
                f"{txt_baseline_acc:.2f}%" if txt_baseline_acc is not None else "N/A"
            )
