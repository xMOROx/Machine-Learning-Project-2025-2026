...
class IntegratedGradients:
    ...
    def __init__(
        self, model: torch.nn.Module, device: str = "cuda", baseline_type: str = "zeros"
    ):
    ...
    def visualize(
        ...
        if not CV2_AVAILABLE:
            raise ImportError(
                "OpenCV (cv2) is required for visualize. Install with: pip install opencv-python"
            )
        ...
        attr_resized = cv2.resize(
            attributions, (original_image.shape[1], original_image.shape[0])
        )
        ...
        attr_norm = (attr_resized - attr_resized.min()) / (
            attr_resized.max() - attr_resized.min() + 1e-8
        )
...
class IntegratedGradientsText:
    ...
    def compute_attributions(
        ...
        for scaled_input in scaled_inputs:
            ...
            gradient = torch.autograd.grad(
                outputs=target_output, inputs=scaled_input, create_graph=False
            )[0]
    ...
    def visualize(
        ...
        ax.set_title(
            f"Predicted: {class_label} (confidence: {confidence:.2%})", fontsize=12
        )
        ax.axis("off")
        ...
        sm.set_array([])
        cbar = plt.colorbar(
            sm, ax=ax, orientation="horizontal", pad=0.02, fraction=0.05
        )
    ...
    def visualize_html(
        ...
        valid_indices = [
            i for i, t in enumerate(tokens) if t not in ["[PAD]", "[CLS]", "[SEP]"]
        ]
