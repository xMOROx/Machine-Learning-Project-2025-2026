...
class GradCAM:
    ...
    def generate_cam_batch(
        self, images: torch.Tensor, target_classes: Optional[torch.Tensor] = None
    ...
    def overlay_heatmap(
        ...
        if not CV2_AVAILABLE:
            raise ImportError(
                "OpenCV (cv2) is required for overlay_heatmap. Install with: pip install opencv-python"
            )
    ...
    def visualize_batch(
        ...
        if not CV2_AVAILABLE:
            raise ImportError(
                "OpenCV (cv2) is required for visualize_batch. Install with: pip install opencv-python"
            )
