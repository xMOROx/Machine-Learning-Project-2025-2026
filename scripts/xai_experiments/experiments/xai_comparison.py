...
class ComparisonConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            k: v if not isinstance(v, list) else v for k, v in self.__dict__.items()
        }
...
class XAIMethodsComparison:
    ...
    def __init__(self, config: Optional[ComparisonConfig] = None):
        ...
        self.results = {
            ...
            "metrics": {},
            "datasets_used": {
                "image": self.config.image_datasets
                if hasattr(self.config, "image_datasets")
                else [self.config.image_dataset],
                "text": self.config.text_datasets
                if hasattr(self.config, "text_datasets")
                else [self.config.text_dataset],
            },
        }
    ...
    def run_image_comparison(
        ...
        image_config = {
            ...
            "comparison_samples": self._get_config_value(
                "image_comparison_samples", 16
            ),
    ...
    def run_text_comparison(
        self, skip_training: bool = False, dataset: str = "sst2"
    ) -> Dict[str, Any]:
    ...
    def generate_summary_report(self) -> str:
        ...
        device = (
            ...
            else self.config.get("device", "cuda")
        )
    ...
    def _create_summary_visualization(self) -> None:
        ...
        num_image_datasets = (
            len(self.results["image_experiments"])
            if self.results["image_experiments"]
            else 0
        )
        num_text_datasets = (
            len(self.results["text_experiments"])
            if self.results["text_experiments"]
            else 0
        )
        ...
        if num_image_datasets > 0:
            ...
            bars1 = axes[0].bar(
                x - width / 2, gradcam_scores, width, label="GradCAM", color="#2196F3"
            )
            bars2 = axes[0].bar(
                x + width / 2, diet_scores, width, label="DiET", color="#4CAF50"
            )

            axes[0].set_ylabel("Pixel Perturbation Score")
            axes[0].set_title(
                "Image Attribution Quality Across Datasets\n(Higher = Better)"
            )
            axes[0].set_xticks(x)
    ...
    def visualize_results(
        self, save_plots: bool = True, show: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self._create_summary_visualization()
            return {
                "summary_plot": os.path.join(self.output_dir, "comparison_summary.png")
            }
