...
class DiETTextExplainer:
    ...
    def train_token_mask(
        ...
        for idx, input_ids, attn_mask, labels, pred_original in data_loader:
            ...
            sparsity = sparsity / attn_mask.float().sum(1)
...
class DiETTextExperiment:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.experiment_name = config.get("experiment_name", f"diet_text_{dataset_name}")
    ...
    def train_baseline(self, epochs: int = 2, save_checkpoint_every: int = 1) -> Dict[str, Any]:
        ...
        if self.checkpoint_manager.has_checkpoint(checkpoint_name):
            ...
            if checkpoint:
                self.model.load_state_dict_from_checkpoint(checkpoint["model_state_dict"])
    ...
    def compare_with_ig(self, num_samples: int = 10) -> Dict[str, Any]:
        ...
        for i in tqdm(range(num_samples), desc="Comparing methods"):
            ...
            try:
                ...
                valid_ig_indices = [
                    j for j, t in enumerate(tokens) if t not in special_tokens
                ]

                valid_diet_indices = [
                    j
                    for j, t in enumerate(diet_tokens)
                    if t not in special_tokens and j < len(diet_attrs)
                ]
