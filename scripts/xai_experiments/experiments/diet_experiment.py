...
class CIFAR10DatasetWithPreds(Dataset):
    ...
    def __init__(self, images: torch.Tensor, labels: torch.Tensor, predictions: torch.Tensor):
        self.images = images
        self.labels = labels
...
class DiETExplainer:
    ...
    def get_predictions(self, images: torch.Tensor, batch_size: int = 64) -> torch.Tensor:
    ...
    def _get_background(self, batch_size: int) -> torch.Tensor:
        ...
        """
        # CIFAR-10 mean/std used for background
    ...
    def train_mask(
        ...
        total_loss = 0
        total_t1 = 0  # Faithfulness to original model
        total_t2 = 0  # Consistency between full and masked
        total_sparsity = 0
...
class DiETExperiment:
    ...
    def prepare_data(self, max_samples: int = 5000) -> None:
        ...
        # Extract tensors for DiET (limited for memory)
        train_images_list = []
        train_labels_list = []
        ...
        # Standard loaders for baseline training
    ...
    def run_diet(self) -> Dict[str, Any]:
        ...
        mask = torch.ones((len(train_preds), 1, mask_size, mask_size), requires_grad=True)
        mask_optimizer = optim.SGD([mask], lr=self.mask_lr)
        model_optimizer = optim.Adam(self.model.parameters(), lr=self.model_lr)
        
        # Rounding scheme for progressive mask binarization
        rounding_scheme = [0.4 - r * (0.4 / self.rounding_steps) for r in range(self.rounding_steps)]
        sparsity_weights = [1 - r * (0.9 / self.rounding_steps) for r in range(self.rounding_steps)]
        ...
        for step in range(self.rounding_steps):
            ...
            print(f"Step {step+1} complete: Test Acc = {diet_history[-1]['model_test_acc']:.2f}%")
    ...
    def compare_methods(self, num_samples: int = 16) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "improvement": np.mean(list(diet_scores.values())) - np.mean(list(gradcam_scores.values()))
        }
    ...
    def _pixel_perturbation_test(
        ...
        with torch.no_grad():
            for i, (attr_map, idx) in enumerate(zip(attribution_maps, indices)):
                ...
                attr_resized = F.interpolate(attr_tensor, size=(32, 32), mode='bilinear', align_corners=False)
                attr_resized = attr_resized.squeeze().numpy()
                
                for p in percentages:
                    # Get threshold for top-p%
    ...
    def _save_comparison_visualizations(
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(viz_dir, "diet_vs_gradcam.png"), dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def run_full_experiment(self) -> Dict[str, Any]:
        ...
        print("=" * 60)
        print(f"Baseline Test Accuracy: {self.results['baseline']['final_test_acc']:.2f}%")
